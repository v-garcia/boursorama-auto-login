...
console.info('Start bourso autologer v4');
...
function log(msg) {
  console.log(`bourso_login : ${msg}`);
}

function buttonToB64Pic($button) {
  const $img = $button.querySelector('img');
  const b64pic = $img.getAttribute('src');
  return b64pic;
}

function digitToButton(buttons, digit) {
  const referencePicture = DIGIT_B64[digit];
  const targetPic = [...buttons].find((el) => buttonToB64Pic(el) === referencePicture);
  return targetPic;
}

(function login() {
  // Password input
  const $pwdInput = document.querySelector('ul.password-input');

  // Digit buttons
  const $digitBtns = $pwdInput?.querySelectorAll('[data-matrix-key]');

  // Wait for url mathing login form and pwd input and digit buttons availables
  // If not, wait and retry
  if (!/\/connexion\/saisie-mot-de-passe(\/)?$/.test(document.URL) || !$pwdInput || !$digitBtns?.length) {
